    ...
    @objc func videoFinishedCallback(notification: NSNotification) {
        if let playerVC = currentVideoPlayerViewController {
            ...
            NotificationCenter.default.removeObserver(
                self,
                name: NSNotification.Name.AVPlayerItemFailedToPlayToEndTime,
                object: playerVC.player)
            
            // Clear up
            clearCurrentVideo()
            
            // Dismiss
            if let errorObj = notification.userInfo?[AVPlayerItemFailedToPlayToEndTimeErrorKey] {
                let error = AVPlayerItemStatus(rawValue: errorObj as! Int)
